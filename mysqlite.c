                 ...
                 const char *errors, const char *metas)
{
    char *sql = sqlite3_mprintf(
        ...
        table_name, node_name, group_name, timestamp, values, errors,
        metas);
...
char *read_records(sqlite3 *db, char *table_name,int threshold){
    ...
    update_sql = sqlite3_mprintf("UPDATE %Q SET uploaded=1 WHERE id=?", table_name);
    if (threshold <= 0) {
        select_sql = sqlite3_mprintf("SELECT * FROM %Q WHERE uploaded=0", table_name);
    } else {
        select_sql = sqlite3_mprintf("SELECT * FROM %Q WHERE uploaded=0 LIMIT ?", table_name);
    }
    ...
    while(sqlite3_step(select_stmt)==SQLITE_ROW){
        ...
        json_array = realloc(json_array,
                             strlen(json_array) + strlen(json_object) + 2);// Add space for comma and null terminator
